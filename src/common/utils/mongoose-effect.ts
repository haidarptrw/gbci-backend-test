 ...
 */
export function mongooseToEffect<T = any>(
  mongooseSchema: MongooseSchema,
): EffectSchema.Schema<T> {
  ...
  mongooseSchema.eachPath((path, schemaType: any) => {
    ...
    if (instance === 'String') baseType = EffectSchema.String;
      ...
      baseType = EffectSchema.Union(
        EffectSchema.Date,
        EffectSchema.String,
        EffectSchema.instanceOf(Date),
      );
  ...
  const dateOrString = EffectSchema.Union(
    EffectSchema.instanceOf(Date),
    EffectSchema.Date,
    EffectSchema.String,
  );
  ...
  if (!shape['createdAt']) {
    shape['createdAt'] = EffectSchema.optional(
      EffectSchema.NullOr(dateOrString),
    );
  }
  if (!shape['updatedAt']) {
    shape['updatedAt'] = EffectSchema.optional(
      EffectSchema.NullOr(dateOrString),
    );
  }
  if (!shape['_id']) {
