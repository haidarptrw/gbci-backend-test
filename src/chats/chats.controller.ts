...
import { Array, Context, Effect, Exit, Schema } from 'effect';
...
export class ChatsController {
  constructor(
    ...
    @Inject(CHAT_REPO_TOKEN)
    private readonly chatRepo: Context.Tag.Service<ChatRepository>,
  ) {}
  ...
  async handleChatMessage(@Payload() data: any, @Ctx() context: RmqContext) {
    ...
    const parsedData = Effect.gen(function* () {
      ...
      if (!parsedDataRes.success) {
        return yield* Effect.fail(
          new BadRequestException('Invalid message data format'),
        );
      }
    ...
    const exit = await Effect.gen(function* () {
      ...
      const savedMessage = yield* repo.createMessage(
        parsedData.chatId,
        parsedData.senderId,
        parsedData.content,
      );
      ...
      .pipe((program) =>
        Effect.provideService(program, ChatRepository, this.chatRepo),
      )
    ...
    const recipient = chat.members
      .map((id) => id.toString())
      .filter((id) => id !== parsedData.senderId);

    recipient.forEach((recipientId) => {
      Effect.gen(this, function* () {
        const senderName = yield* Schema.decodeUnknown(UserSchema_Effect)(
          savedMessage.sender,
        ).pipe(
